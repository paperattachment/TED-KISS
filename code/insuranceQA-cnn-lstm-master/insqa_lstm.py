...
def load_data(trainList, vocab, batch_size):
    ...
    while True:
        ...
        if pos[2].startswith('<a>') or pos[3].startswith('<a>') or neg[3].startswith('<a>'):
            #print 'empty string ......'
            continue
        x, m = encode_sent(vocab, pos[2], 200)
        train_1.append(x)
        mask_1.append(m)
        x, m = encode_sent(vocab, pos[3], 200)
        train_2.append(x)
        mask_2.append(m)
        x, m = encode_sent(vocab, neg[3], 200)
        train_3.append(x)
        mask_3.append(m)
        counter += 1
...
def load_data_val(testList, vocab, index, batch_size):
    ...
    for i in range(0, batch_size):
        ...
        x, m = encode_sent(vocab, items[2], 200)
        x1.append(x)
        m1.append(m)
        x, m = encode_sent(vocab, items[3], 200)
        x2.append(x)
        m2.append(m)
        x, m = encode_sent(vocab, items[3], 200)
        x3.append(x)
...
def validation(validate_model, testList, vocab, batch_size, test_num):
    ...
    for qid, cases in sdict.items():
        ...
        for i in range(0,len(cases)):
            ...
            #f_test.write('qid:'+str(qid)+' '+'flag:'+str(flag)+' '+'score:'+str(score)+'\n')
            f_test.flush()
        #"""    
        """
         score, flag = cases[0]      
...
def param_init_lstm(proj_size, tparams, grad_params):
    #axis=0:col(up-down);axis=1:row(left,right)   
